...
import { transcriptAnalyzer } from './gpt4-analyzer.js';
    ...
    mediaWs.on('message', (data) => {
        try {
            ...
            if (msg.msg_type === 17) {
                ...
                // Debug: Log the full content structure to understand speaker data
                console.log('🔍 TRANSCRIPT CONTENT STRUCTURE:', {
                    content: msg.content,
                    contentKeys: msg.content ? Object.keys(msg.content) : [],
                    userName: msg.content?.user_name,
                    speakerName: msg.content?.speaker_name,
                    displayName: msg.content?.display_name,
                    participantName: msg.content?.participant_name,
                });

                // Try multiple possible speaker name fields
                const speakerName = msg.content?.user_name || 
                                  msg.content?.speaker_name || 
                                  msg.content?.display_name || 
                                  msg.content?.participant_name ||
                                  msg.user_name ||
                                  msg.speaker_name ||
                                  'Unknown';
                ...
                // Analyze transcript with GPT-4 for task creation opportunities
                transcriptAnalyzer.analyzeForTasks(meetingUuid, transcriptData)
                    .then(taskSuggestion => {
                        if (taskSuggestion) {
                            console.log('🎯 TASK SUGGESTION DETECTED:', taskSuggestion);
                            // Send both transcript and task suggestion
                            onTranscript(meetingUuid, {
                                ...transcriptData,
                                taskSuggestion
                            });
                        } else {
                            // Send just the transcript
                            onTranscript(meetingUuid, transcriptData);
                        }
                    })
                    .catch(error => {
                        console.error('❌ GPT-4 ANALYSIS ERROR:', error);
                        // Send transcript even if analysis fails
                        onTranscript(meetingUuid, transcriptData);
                    });
            }

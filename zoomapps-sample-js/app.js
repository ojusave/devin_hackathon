...
import rtmsRoutes, { setBroadcastFunction } from './server/routes/rtms.js';
...
// Start serving and return HTTP server instance for WebSocket integration
start(app, port)
    .then((server) => {
        console.log(`✅ HTTP server running on port ${port}`);
        console.log('🔌 WebSocket server integrated for transcript streaming');
        
        // Connect the broadcast function from server to RTMS routes
        if (server.broadcastTranscript) {
            setBroadcastFunction(server.broadcastTranscript);
            console.log('📡 RTMS routes connected to global broadcast function');
        } else {
            console.warn('⚠️ No broadcast function found on server');
        }
    })
    .catch(async (e) => {
        console.error(e);
